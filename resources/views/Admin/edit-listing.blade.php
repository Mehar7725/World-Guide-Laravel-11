  ...
  <body class="relative bg-gray-100">
    ...
    <div class="bg-gray-100 flex flex-col items-center justify-center">
      <div class="w-[90%] max-w-[1000px] text-center">
        <!-- Title -->
        <h1 class="text-2xl font-semibold text-gray-800">Edit Add Listing</h1>
        <!-- Subtitle -->
        <p class="text-gray-500 mt-1">Edit details about your listing</p>
        <!-- Divider -->
        ...
        <div class="w-[80%] overflow-y-auto h-[90vh]">
          <div class="w-full bg-white p-6 shadow-md rounded-lg">
            ...
            <div class="mb-4 w-full">
              ...
              <div class="flex items-center w-full" id="by-address">
                <input
                  type="text"
                  id="address" name="address" value="{{$add->address}}"
                  placeholder="Start typing and find your place in google map"
                  class="w-full p-2 border border-gray-300 rounded-l"
                />
              </div>
          ...
          <div class="bg-white shadow-md rounded px-8 py-6 mb-4">
            ...
            <div class="flex mb-4">
              <div class="flex-1 mr-4">
                ...
                <input
                  type="text" value="{{$add->price_details}}"
              ...
              <div class="flex-1 mr-4">
                ...
                >
                <input
                  type="text" value="{{$add->price_from}}"
          ...
          <div class="bg-white shadow-md rounded px-8 py-6 mb-4">
            ...
            <div class="flex flex-col" id="full_availibility_div">
              ...
              @php
              $availability = $add->availability ? json_decode($add->availability, true) : [];
          @endphp
          
          @if (!empty($availability))
              @foreach ($availability as $slot)
                  <div class="flex w-full justify-between border-b-[1px] border-gray-200 py-3">
                      <div class="w-[90px]">{{ $slot['day'] }}</div>
          
                      <div class="">
                          @if($slot['full_available'] == 1)
                              24 Hours Available
                          @else
              ...
              @endforeach
          @else
              <p class="text-gray-500">No availability data found.</p>
          @endif
            ...
            <div class="w-full flex justify-between items-center py-2">
              ...
              <input type="hidden" value="{{$add->availability}}" name="availability" id="availability">
          ...
          <div class="bg-white shadow-md rounded px-8 py-6 mb-4">
            ...
            >
              ...
              <div class="relative hidden group" id="image-preview-container">
                <img id="image-preview" src="" class="w-32 h-32 object-cover rounded-lg" />
...
{{-- Show preview Images selection script Start ======= --}}
    <script>
   function setupImageUpload(inputId, previewId, containerId, textId, removeBtnId, existingSrc) {
    ...
    // Check if there is an existing image and set it on page load
    if (existingSrc) {
...
// Set the image paths from Laravel Blade variables
const userId = `{{ $add->user_id }}`;
const basePath = `/assets/adds/data_${userId}/`;

const imaged = @json($add->image);
const featuredImaged = @json($add->featured_image);
const logod = @json($add->logo);

const imagePath = imaged ? `${basePath}${imaged}` : "";
const featuredImagePath = featuredImaged ? `${basePath}${featuredImaged}` : "";
const logoPath = logod ? `${basePath}${logod}` : "";
 

// Setup functionality for all three image inputs
setupImageUpload("image", "image-preview", "image-preview-container", "image-text-content", "image-remove-btn", imagePath);
setupImageUpload("featured_image", "featured-preview", "featured-preview-container", "featured-text-content", "featured-remove-btn", featuredImagePath);
setupImageUpload("logo", "logo-preview", "logo-preview-container", "logo-text-content", "logo-remove-btn", logoPath);
...
$(document).ready(function () {
    ...
    // Load existing FAQs from the hidden input on page load
    let existingFaqs = $("#all_faq").val();
    if (existingFaqs) {
        faqs = JSON.parse(existingFaqs);

        // Append each existing FAQ to the UI
        faqs.forEach(faq => {
            let uniqueId = Date.now() + Math.floor(Math.random() * 1000); // Ensure unique ID
            faq.id = uniqueId; // Assign an ID to track removal
        ...
        // Update hidden input after assigning IDs
        updateFaqInput();
    }
...
<script>
  $(document).ready(function () {
    ...
    // Load existing data from the hidden input
    let existingLinks = $("#all_links").val();
    if (existingLinks) {
        socialLinks = JSON.parse(existingLinks);
        
        // Populate UI with existing links and remove options from dropdown
        socialLinks.forEach(item => {
            $("#social-links-list").append(`
                <div class="flex justify-between items-center border p-2 rounded shadow-md" data-platform="${item.platform}">
                    <span class="font-semibold">${item.platform}:</span>
                    <a href="${item.url}" target="_blank" class="text-blue-600">${item.url}</a>
                    <button class="remove-link text-red-500 ml-3" data-platform="${item.platform}">
                        <i class="fa-solid fa-times"></i>
                    </button>
                </div>
            `);
            // Remove platform from dropdown
            $("#select_social_media option[value='" + item.platform + "']").remove();
        });
    }
...
<script>
  document.addEventListener("DOMContentLoaded", function () {
      ...
      let availabilityData = @json(json_decode($add->availability, true) ?? []);
      let selectedDays = availabilityData.map(item => item.day); // Days from DB
      let manuallyAddedDays = new Set(); // Track manually added days

      // 🛑 Remove days from the dropdown on page load (DB values only)
      function removeDbDays() {
          selectedDays.forEach(day => {
              let option = document.querySelector(`#select_day option[value="${day}"]`);
              if (option) option.remove();
          });
      }
      removeDbDays(); // Run on page load
    ...
    addButton.addEventListener("click", function () {
        ...
        if (!isFullAvailable && !isValidTime(startTime, endTime)) {
          ...
          manuallyAddedDays.add(selectedDay); // Track manually added day

          // Remove selected day from dropdown
          let option = document.querySelector(`#select_day option[value="${selectedDay}"]`);
          if (option) option.remove();
      ...
      // 🛑 Remove availability entry when clicking remove button
      $(document).on("click", ".remove_availability", function () {
          ...
          // 🔹 Add back only if it was manually added (not from DB)
          if (manuallyAddedDays.has(removedDay)) {
              $("#select_day").append(`<option value="${removedDay}">${removedDay}</option>`);
              manuallyAddedDays.delete(removedDay);
          }
...
{{-- On Page Load Remove Selected Days From Availibility selection days list --}}
<script>
  document.addEventListener("DOMContentLoaded", function () {
      // Sample JSON data (Replace this with actual data from backend)
      let availability = @json(json_decode($add->availability, true) ?? []);

      // Extract days from availability
      let selectedDays = availability.map(slot => slot.day);

      // Remove already selected days from dropdown
      let selectBox = document.getElementById("select_day");

      for (let option of selectBox.options) {
          if (selectedDays.includes(option.value)) {
              option.remove(); // Remove option if day is already selected
          }
      }
  });
</script>
